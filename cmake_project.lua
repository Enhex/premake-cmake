...
function m.getcompiler(cfg)
    ...
    if not toolset then error("Invalid toolset '" + (_OPTIONS.cc or cfg.toolset) + "'") end
    return toolset
end
...
function m.generate(prj)
    ...
    if prj.kind == nil then for cfg in project.eachconfig(prj) do prj.kind = cfg.kind end end
    ...
    for cfg in project.eachconfig(prj) do
        ...
        if ok then
            ...
            if ok == nil then printf("Unable to create directoy: %s", err) end
        end
        ...
        if all_build_options ~= "" then
            _p("if(CMAKE_BUILD_TYPE STREQUAL %s)", cmake.cfgname(cfg))
            _p(1, "set_target_properties(\"%s\" PROPERTIES COMPILE_FLAGS \"%s\")", prj.name,
               all_build_options)
            _p("endif()")
        end
        ...
        if all_link_options ~= "" then
            _p("if(CMAKE_BUILD_TYPE STREQUAL %s)", cmake.cfgname(cfg))
            _p(1, "set_target_properties(\"%s\" PROPERTIES LINK_FLAGS \"%s\")", prj.name,
               all_link_options)
            _p("endif()")
        end
        ...
        if not cfg.flags.NoPCH and cfg.pchheader then
            ...
            _p("if(CMAKE_BUILD_TYPE STREQUAL %s)", cmake.cfgname(cfg))
            _p("target_precompile_headers(\"%s\" PUBLIC %s/%s)", prj.name, prj.name, pch)
            _p("endif()")
        end
